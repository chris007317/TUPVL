$(document).ready(function(){
	...
	datos.append("funcion", 'veDisponibilidad');
	verBeneficiarios(datos, '#btnRegistrarPostulante', 'ajax/municipalidad.ajax.php');
...
$(document).on('change', '#cmbTipoPostulante', function(){
	...
	if (idTipoPost > 0) {
		...
		if (valReg == true) {
			tiempoMeses = $(this).find("option[value='"+idTipoPost+"']").attr('tiempoMeses');
			$('input[name="mesesMaximo"]').val(tiempoMeses);
			if (idTipoPost == 1 || idTipoPost == 4) {
				$("#madres").css("display", "flex");
			}else{
				$("#madres").css("display", "none");
			}
		}else{
			$("#madres").css("display", "none");
			$('input[name="mesesMaximo"]').val('');
		}
	}
});
...
$(document).on('change', 'input[name="dateFechaNac"]', function(){
	let fecha = $(this).val();
	let idTipoPost = $('#cmbTipoPostulante').val();
...
$(document).on('click', '#btnRegistrarPostulante', function(event){
	if (validarBen == 1) {
		...
		if (!$('input[name="customRadio"]').is(':checked')) {
			...
			event.preventDefault();
		}else if(idTipoPost == 1 || idTipoPost == 4){ 
			let meses =  parseInt($('input[name="txtMeses"]').val());
			let dias =  parseInt($('input[name="txtDias"]').val());
			if (meses >= tiempoMeses || dias > 31) {
				mensaje('¡Advertencia!', '¡El tiempo de del bebé consignado supera al permitido!', 'warning');
				event.preventDefault();
			}
		}
	}
